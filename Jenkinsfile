    ...
    stages {
       stage('Build') {
		   ...
		   node('master'){
			   ...
			   sh label: '', script: 'sudo docker push 18.218.171.29:8888/devi:1.0.1'
			   }
		   node('slave1'){	
		       sh label: '', script: 'sudo docker login -u admin -p admin123 52.14.54.48:8888'
		       sh label: '', script: 'sudo docker pull 52.14.54.48:8888/devi:1.0.1'
			   sh label: '', script: 'sudo docker service create --name qt-raju --network qt-overlay --replicas 3  -it -d -p 9999:8080 52.14.54.48:8888/devi:1.0.1'
           }
		}
